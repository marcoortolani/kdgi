...
BlueStar::BlueStar(const char * path, double alpha_value, double delta_value):BlueFringe(path){
	alpha = alpha_value;
	delta = delta_value;
};
...
double BlueStar::merge_heuristic_score(double error_rate_before, double error_rate_after, int dim_strings, double alpha, double earn_states)
{
	double z_alpha 		= DMAX;
	double T			= error_rate_after - error_rate_before;
	double dev_std_h0 	= DMAX;
	//cout<<"DEBUG: z_alpha="<<z_alpha<<"dev_std_h0="<<dev_std_h0<<endl;	//OK
	...
	else
	{
		...
		if(earn_states != 0)
			ratio_error_to_n_states = cdf(s,zeta_beta)/(double) earn_states;
		else
			ratio_error_to_n_states = cdf(s,zeta_beta);


		#ifdef DEBUG1
		cout << "T: " << T << ", z_alpha: "<<z_alpha<<", delta: "<<delta<< "dev_std: "<<dev_std_h0<< endl;
		cout << "zeta-beta: "<< zeta_beta << endl;
		cout << "probability: "<< cdf(s,zeta_beta) << endl;
...
Dfa* BlueStar::run(string base_path)
{
	...
	RedBlueDfa *dfa1 =NULL,  **merged = NULL; // dfa...
...
double BlueStar::get_error_rate_final_dfa()
{
