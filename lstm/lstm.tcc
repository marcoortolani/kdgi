...
std::vector<int> vec_sym_to_vec_int(std::vector<std::string> phrase, std::vector<std::string> alphabet){
	...
	for(std::string sym : phrase){
		...
		if(unidentified_symbol){
			std::cerr << "Error in LSTMOracle::vec_sym_to_str: found the unidentified symbol\"" << sym << "\"" << std::endl;
			throw 0;
		}
	}
	return vec_int;
...
bool LSTMOracle::add_word(std::vector<symbol_> word){
	for(auto w : words_){
		if(w == word){
...
LSTMOracle::LSTMOracle(int layer, std::vector<std::string> alphabet, int max_build){
	alphabet_ = alphabet;
	//net_ = rnn;
	layer_ = layer;
	A_ = nullptr;
	...
	py::module lstm_module = py::module::import("LSTMOracle");
	net_ = lstm_module.attr("LSTMOracle")("model6b");
