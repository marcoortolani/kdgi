...
std::vector<int> vec_sym_to_vec_int(std::vector<std::string> phrase, std::vector<std::string> alphabet){
	...
	for(std::string sym : phrase){
		...
		if(unidentified_symbol){
			std::cerr << "Error in RNNOracle::vec_sym_to_str: found the unidentified symbol\"" << sym << "\"" << std::endl;
			throw 0;
		}
	}
	return vec_int;
...
vector<vector<symbol_>> RNNOracle::get_words_by_length(int min_len, int max_len){
...
std::vector<double> RNNOracle::get_layer_output(std::vector<int> net_input){
...
int RNNOracle::get_state_index_from_word(std::vector<std::string> phrase){
...
bool RNNOracle::add_word(std::vector<symbol_> word){
	for(auto w : words_){
		if(w == word){
	...
	symbol_ transition = prefix.back();
	prefix.pop_back();
	
	int start_state = A_->get_arrive_state(prefix);
	int arrive_state = words_.size();
	mandatory_transitions_[make_pair(start_state, transition)] = arrive_state;
...
template <class Dfa>
pair<vector<symbol_>,vector<symbol_>> RNNOracle::breadth_search(Dfa* dfa_hp, int max_len){
	...
	for(int i = 0; i < max_len; i++){
		...
		for(int j = 0; j < c; j++){
			...
			DfaState* AState = (*A_)[word];
			DfaState* learnerState = (*dfa_hp)[word];
			auto it = state_recorder.find(AState->get_charact_phrase());
			if(it == state_recorder.end()){
				state_recorder[AState->get_charact_phrase()] = make_pair(learnerState->get_charact_phrase(), word);
			}
			else{
...
void RNNOracle::build_dfa(){
	...
	for(int i = 0; i < len; i++){
		...
		for(auto sym : alphabet_){
			...
			auto it = mandatory_transitions_.find(make_pair(i, sym));
			if(it == mandatory_transitions_.end()){
...
bool RNNOracle::membership_query(std::vector<std::string> phrase){
